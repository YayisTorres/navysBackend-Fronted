...
@Component({
  ...
  template: `
    ...
    <!-- Hero Section específico para Bautizos -->
    <section class="hero hero-bautizos">
      <div class="hero-bg">
        <img
          src="https://navys-5eeb9.web.app/img/imagenesinicio/xv%20carrucel%20damas.jpeg"
          alt="Bautizos"
          class="hero-image"
        />
      ...
      <div class="hero-content">
        <h1 class="hero-title">Bautizos</h1>
        <p class="hero-subtitle">
          Elegancia atemporal para el día más especial de tu pequeño.
        </p>
        <div class="hero-stats">
          <div class="stat">
            <span class="stat-number">{{ bautizosProducts.length }}</span>
    ...
    <section class="products-section">
      <div class="container">
        <div class="section-header">
          <h2 class="section-title">Colección Bautizos</h2>
          <p class="section-subtitle">
            Vestidos y trajes especiales para la ceremonia más importante
          </p>
        </div>
        ...
        <div
          class="products-grid"
          *ngIf="!loading && bautizosProducts.length > 0"
        >
          <app-product-card
            *ngFor="let product of bautizosProducts"
            [product]="product"
        ...
        <div class="loading-products" *ngIf="loading">
          ...
          <p>Cargando productos de Bautizos...</p>
        </div>
        ...
        >
          ...
          <p>No se encontraron productos de Bautizos en la base de datos.</p>
        </div>
      </div>
    </section>
...
export class BautizosComponent implements OnInit {
  ...
  bautizosProducts: ProcessedProduct[] = [];

  ngOnInit(): void {
    this.loadBautizosProducts();
  }

  // 🔥 SOLO MÉTODOS DE PRODUCTOS - El header se maneja en HeaderComponent
  loadBautizosProducts(): void {
    this.loading = true;
    ...
    this.productService.getAllProducts().subscribe({
      next: (response: any) => {
        ...
        if (allProducts.length === 0) {
          ...
          this.bautizosProducts = [];
          this.loading = false;
          return;
        }

        // Filtrar productos que contengan "bautizo" en la categoría
        const filteredProducts = allProducts.filter(
          (product: Product) =>
            product.category?.toLowerCase().includes('bautizo') ||
            product.category?.toLowerCase().includes('bautismo')
        );

        // Procesar productos filtrados
        this.bautizosProducts = filteredProducts.map((product: Product) =>
          this.processProductData(product)
        );
        console.log('🔥 Processed Bautizos products:', this.bautizosProducts);
        this.loading = false;

        this.loadFavoritesStatus();
      },
      error: (error: any) => {
        console.error('Error cargando productos de Bautizos:', error);
        this.bautizosProducts = [];
        this.loading = false;
      },
    });
  ...
  processProductData(product: Product): ProcessedProduct {
    console.log('🔥 Processing Bautizos product:', product.name, product);
  ...
  loadFavoritesStatus(): void {
    ...
    this.bautizosProducts.forEach((product) => {
  ...
  onFavoriteToggled(event: { productId: string; isFavorite: boolean }): void {
    ...
    const product = this.bautizosProducts.find((p) => p.id === event.productId);
    console.log(`✅ ${product?.name} ${action} favoritos`);
  }

  onAddedToCart(product: Product): void {
    console.log(
      '🛒 Producto agregado al carrito desde Bautizos:',
      product.name
    );
  }

  onProductClicked(product: Product): void {
    console.log('👁️ Ver producto Bautizos:', product.name);
  }
}
